...
def test_create(client, feed_with_posts, stixifier_profile, api_schema):
    ...
    with (
        ...
        api_schema['/api/v1/feeds/']['POST'].is_valid_response(Transport.get_st_response(None, resp))
...
def test_fetch(client, feed_with_posts, stixifier_profile, api_schema):
    ...
    with (
        ...
        assert resp.data["id"] == str(mocked_job.id)
        api_schema['/api/v1/feeds/{feed_id}/fetch/']['PATCH'].is_valid_response(Transport.get_st_response(None, resp))
...
def test_feed_destroy(client, feed_with_posts, api_schema):
    ...
    api_schema['/api/v1/feeds/{feed_id}/']['DELETE'].is_valid_response(Transport.get_st_response(None, resp))
