...
def all_posts():
    DATA = [
        ...
        },
        {
            ...
            "author": "test",
            "categories": ["test", "test2"],
            "should_fail": True,  # already added, also no profile_id
        },
    ]
    return [[d["feed_id"], d["link"], d, d.get("should_fail")] for d in DATA]


@pytest.mark.parametrize(
    ["feed_id", "post_url", "post_data", "should_fail"], all_posts()
)
def test_add_post(feed_id, post_url, post_data, should_fail):
    payload = remove_unknown_keys(
        post_data, ["link", "title", "pubdate", "author", "categories"]
    )
    post_job_resp = requests.post(
        urljoin(base_url, f"api/v1/feeds/{feed_id}/posts/"), json=dict(posts=[payload], profile_id=post_data.get("profile_id"))
    )
    ...
    assert set(post_data_resp_data["categories"]) == set(
        post_data.get("categories", [])
    )
    assert post_data_resp_data['profile_id'] == post_data['profile_id']
